...
void printVerticalOrderRec(Node *root, int hd,map<int,vector<int>> &res)
{
	if(!root)
		return;
	
	res[hd].push_back(root->key);
	printVerticalOrderRec(root->left,hd-1,res);
	printVerticalOrderRec(root->right,hd+1,res);
...
int main()
{
    ...
    map<int,vector<int>> m;
    printVerticalOrderRec(root,0,m);
    print(m);
    return 0;
}
